<?php
// TODO: when php 5.4 most of this trait along with code in drawing
class MaterialController extends Controller
{
	...
	private function initUploadHandler($id)
	{
		...
		$uploadDir = Yii::app()->params['privateUploadPath'] . "material/$id/";
		$session_id = session_id();
		exec("mkdir " . Yii::app()->params['privateUploadPath'] . "material");
		exec("mkdir " . Yii::app()->params['publicUploadPath'] . "material");
		exec("mkdir $uploadDir");

		Yii::import("xupload.UploadHandler");
		$this->uploadHandler = new UploadHandler(array(
			'upload_dir' => $uploadDir,
			'upload_url' => Material::model()->findByPk($id)->expose,
	...
	public function create($modal_id = 'myModal') {
		$this->initUploadHandler($_POST['Material']['created']);
	}
	...
	public function actionCreate($modal_id = 'myModal', &$model = null)
	{
		...
		if(isset($_POST[$this->modelName]))
		{
			...
			// if an error occurrs in file upload after ajax validation has already created the standard material record then we really wan't actionUpdate
...
// TODO: potentially not maintaining relational integrity here
		...
		$source = Yii::app()->params['privateUploadPath'] . "material/$id";
		// remove it if exists
		exec("rm -rf $source");
