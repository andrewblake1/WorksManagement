...
$form=$this->beginWidget('WMTbActiveForm', array('model'=>$model, 'parent_fk'=>$parent_fk));		
	...
	SupplierController::listWidgetRow($model, $form, 'supplier_id',
		array(
			...
			'ajax' => array(
			'type'=>'POST',
			'url'=>CController::createUrl('Supplier/dynamicMaterials'),
			'success'=>"function(data) {
				if(data)
				{
					...
					if(lookup.length)
					{
						...
						lookup.autocomplete({'minLength':1,'maxHeight':'100','select':function(event, ui){"."$('#{$htmlOptions['id']}').val(ui.item.id);$('#{$htmlOptions['id']}_save').val(ui.item.value);},'source':'/WorksManagement/Material/autocomplete?model={$this->modelName}&attribute=material_id&{$this->modelName}%5Btask_id%5D=35&scopes%5BscopeSupplier%5D%5B0%5D=' + supplier_id});
					}
				}
			}",
		)),
	...
	MaterialController::listWidgetRow($model, $form, 'material_id', array(), array('scopeSupplier'=>array($model->supplier_id)));
