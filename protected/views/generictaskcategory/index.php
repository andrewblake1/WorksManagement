...
echo '
	...
	<div id="'.Generictaskcategory::ADMIN_TREE_CONTAINER_ID.'" >
...
<script  type="text/javascript">
	$(function ()
	{
		$("#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>")
		.jstree(
		{
			"html_data" :
				{
				"ajax" :
					{
					"type":"POST",
					"url" : "<?php echo $baseUrl; ?>/generictaskcategory/fetchTree",
					"data" : function (n)
					{
						return{
			...
			"contextmenu":  { 'items': {
					...
					"update" : {
						...
						"action"	: function (obj) {
							...
							$.ajax({
								type: "POST",
								url: "<?php echo $baseUrl; ?>/generictaskcategory/returnForm",
								data:{
									'update_id':  id,
									"YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken; ?>"
								},
								'beforeSend' : function(){
									$("#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>").addClass("ajax-sending");
								},
								'complete' : function(){
									$("#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>").removeClass("ajax-sending");
								},
	...
	//								$('[name="parent_id"]').val(obj.attr("id").replace("node_",""));
					...
					"remove" : {
						...
						"action" : function (obj) {
							...
							{
								jQuery("#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>").jstree("remove",obj);
							}
						}
					},//remove
					"create" : {
		...
		.bind("rename.jstree", function (e, data) {
			$.ajax({
				type:"POST",
				url:"<?php echo $baseUrl; ?>/generictaskcategory/rename",
				data:  {
		...
		.bind("remove.jstree", function (e, data) {
			$.ajax({
				type:"POST",
				url:"<?php echo $baseUrl; ?>/generictaskcategory/remove",
				data:{
					"id" : data.rslt.obj.attr("id").replace("node_",""),
					"YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken; ?>"
				},
		...
		.bind("move_node.jstree", function (e, data) {
			data.rslt.o.each(function (i) {
				...
				next= jQuery.jstree._reference('#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>')._get_next (this, true);
				previous= jQuery.jstree._reference('#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>')._get_prev(this,true);
				...
				$.ajax({
					async : false,
					type: 'POST',
					url: "<?php echo $baseUrl; ?>/generictaskcategory/moveCopy",
		...
		{
			jQuery("#<?php echo Generictaskcategory::ADMIN_TREE_CONTAINER_ID; ?>").jstree("refresh");
		});
	});
