...
<div id="schedule_form_con"   class="client-val-form">
<?php if ($_POST['create_root']=='true' && $model->isNewRecord) : ?>              <h3 id="create_header">Create New Root Schedule </h3>
<?php elseif ($model->isNewRecord) : ?>     <h3 id="create_header">Create New Schedule </h3>
     <?php  elseif (!$model->isNewRecord):  ?>     <h3 id="update_header">Update Schedule <?php  echo $model->name;  ?> (ID:<?php   echo $model->id;  ?>) </h3>
    <?php   endif;  ?>
    <div   id="success-schedule" class="notification success png_bg" style="display:none;">
				<a href="#" class="close"><img src="<?php echo Yii::app()->request->baseUrl.'/css/images/icons/cross_grey_small.png';  ?>"
                                                                title="Close this notification" alt="close" /></a>
			</div>

<div  id="error-schedule" class="notification errorshow png_bg" style="display:none;">
				<a href="#" class="close"><img src="<?php echo Yii::app()->request->baseUrl.'/css/images/icons/cross_grey_small.png';  ?>"
                                                                title="Close this notification" alt="close" /></a>
			</div>
...
<?php   $formId='schedule-form';
 $ajaxUrl=($model->isNewRecord)?
              ( ($_POST['create_root']!='true')?CController::createUrl('schedule/create'):CController::createUrl('schedule/createRoot')):
               CController::createUrl('schedule/update');
$val_error_msg='Error.Schedule was not saved.';
$val_success_message=($model->isNewRecord)?
( ($_POST['create_root']!='true')?'Schedule was created successfuly.':'Root Schedule was created successfuly.'):
                                                  'Schedule was updated successfuly.';
...
$success='function(data){
    ...
    {
         jQuery("#'.Schedule::ADMIN_TREE_CONTAINER_ID.'").jstree("refresh");
         $("#success-schedule")
        ...
        $("#schedule-form").slideToggle(1500);'.
        $updatesuccess.
    '}
         else {
                   $("#error-schedule")
     ...
     'clientOptions'=>array('validateOnSubmit'=>true,
                                        ...
                                        'afterValidateAttribute' => 'js:function(form, attribute, data, hasError){
                                                   if(!hasError){
                                                                   ...
                                                                   $("label[for=\'Schedule_"+attribute.name+"\']").removeClass("error");
                                                                      }else {
                                                                                  $("label[for=\'Schedule_"+attribute.name+"\']").addClass("error");
...
<?php echo $form->errorSummary($model, '<div style="font-weight:bold">Please correct these errors:</div>', NULL, array('class' => 'errorsum notification errorshow png_bg')); ?><p class="note">Fields with <span class="required">*</span> are required.</p>
 ...
 <div class="row" >
  <?php echo $form->labelEx($model,'name'); ?>    <?php  echo $form->textField($model,'name',array('size'=>60,'maxlength'=>128,'value'=>$_POST['name'],'style'=>'width:75%;'));  ?>       <span  id="success-Schedule_name"  class="hid input-notification-success  success png_bg"></span>
    <div><small></small> </div>
	...
	<div class="row">
		...
		<?php echo $form->textField($model,'staff_id'); ?>
             <span  id="success-Schedule_staff_id"  class="hid input-notification-success  success png_bg"></span>
           <div><small></small> </div>
