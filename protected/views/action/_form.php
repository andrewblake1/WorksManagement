...
$form=$this->beginWidget('WMTbActiveForm', array('model'=>$model, 'parent_fk'=>$parent_fk));
	...
	if(isset($_GET['project_template_id']))
	{
		...
		static::listWidgetRow($model, $form, 'override_id', array(), array('scopeProjectTemplate'=>array($model->project_template_id)), 'Replaces');
	}
	// otherwise if a client is given
	elseif(isset($_GET['client_id']))
	{
		// scope to only allow where both project template and client are null
		static::listWidgetRow($model, $form, 'override_id', array(), array('scopeClient'=>array($model->client_id)), 'Replaces');
	}
